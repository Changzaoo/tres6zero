...
async function activatePixGoPayment(record: StoredPixGoPayment, sourceEventId?: string) {
  ...
  await createNotification({
    ...
    body: `Seu plano ${record.planName} esta ativo ate ${nextEnd.toLocaleDateString('pt-BR')}.`,
    link: '/app/billing',
    priority: 'high',
...
export async function createPixGoPayment(user: AuthenticatedBillingUser, planId: BillingPlanId) {
  ...
  const requestBody: Record<string, unknown> = {
    ...
    customer_name: user.displayName || user.email || 'Usuario SIX3',
  ...
  const record: StoredPixGoPayment = {
    ...
    userName: user.displayName || user.email || 'Usuario SIX3',
    planId: plan.id,
    planName: plan.name,
